...
#if fvec2
	Vec vals_coupling_fvec;
	VecCreate(PETSC_COMM_WORLD,&vals_coupling_fvec);
  PetscObjectSetName((PetscObject) vals_coupling_fvec, "Solution");
  VecSetSizes(vals_coupling_fvec,PETSC_DECIDE,3*a_nrow+t_nrow);
  VecSetFromOptions(vals_coupling_fvec);
  VecSetFromOptions(vals_coupling_fvec);
  VecAssemblyBegin(vals_coupling_fvec);
  VecAssemblyEnd(vals_coupling_fvec);
#endif
...
#if fvec2
	VecSetValue(vals_coupling_fvec, idx_count_f, - constant,ADD_VALUES);
#endif
...
#if fvec2
VecSetValue(vals_coupling_fvec, idx_count_f, -elem->volume()/(omega_end_j_f[closest_end_edge_j+1]),ADD_VALUES); 
#endif
...
#endif
#if fvec2
		VecSetValue(vals_coupling_fvec,idx_count_f,1,ADD_VALUES); 
#endif
