  ...
  std::cout<< "looooa "<<std::endl;
	...
	{    	
	  ...
	  found_end_j[closest_end_edge_j+1]=found_end_j[closest_end_edge_j+1]+1;
		...
		/*
		rows_coupling[idx_count]=dof_indices_p[0];
    cols_coupling[idx_count]=AP.m()+tree.number_nodes+closest_end_edge_j;
		vals_coupling[idx_count*(a_nrow+t_nrow) + idx_count]= - constant; 
		...
		*/
		...
		//rows_coupling[idx_count]=AP.m()+tree.number_nodes+ closest_end_edge_j;
   // cols_coupling[idx_count]=dof_indices_p[0];
		//vals_coupling[idx_count*(a_nrow+t_nrow) + idx_count]= -elem->volume()/(omega_end_j[closest_end_edge_j+1]); 
		//idx_count=idx_count+1;		
		...
		if(already_set[closest_end_edge_j+1]==0){
		  ...
		  MatSetValue(big_A, AP.m()+tree.number_nodes+ closest_end_edge_j , AP.m()+closest_end_edge_j +1 , 1 ,ADD_VALUES); 		  
			...
			//rows_coupling[idx_count]=AP.m()+tree.number_nodes+ closest_end_edge_j;
			//cols_coupling[idx_count]=AP.m()+closest_end_edge_j +1;
			//vals_coupling[idx_count*(a_nrow+t_nrow) + idx_count]=1; 
			//idx_count=idx_count+1;	
	...
	//INsert values
//	MatSetValues(big_A,a_nrow, rows_coupling,t_nrow, cols_coupling,vals_coupling,ADD_VALUES);
...
/*
	 ...
	 for ( int i = 0; i < a_nrow+t_nrow; i++ )
  {
   ...
   std::cout<< "cols_coupling[i]" <<std::endl;
  for ( int i = 0; i < a_nrow+t_nrow; i++ )
  {
			std::cout<< cols_coupling[i] <<std::endl;
  }
  
  std::cout<< "vals_coupling[i]" <<std::endl;
  for ( int i = 0; i < 2*(a_nrow+t_nrow); i++ )
  {
			std::cout<< vals_coupling[i] <<std::endl;
  }
