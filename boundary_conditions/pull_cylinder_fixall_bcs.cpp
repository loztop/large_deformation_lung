...
//constrain bottom in all directions - let rest slide
...
//Fix all outside nodes // Let slide !!
       ...
       if ((elem->node(n) == side->node(ns)) && ( (zf<0.001) &&  (xf>0.99) &&  (yf<0.1)  &&  (yf>-0.1) ))
        {
          ...
          if ((elem->node(n) == side->node(ns)) && (  (zf>0.999) ))
          {
            ...
            if((source_dof<12345678) && (source_dof>-1)){
							...
							Real value = last_non_linear_soln.current_local_solution->el(source_dof) - ref_sys.current_local_solution->el(source_dof)-progress*0.02;
						rows_values.push_back(value);
						 rows.push_back(source_dof);
            }
