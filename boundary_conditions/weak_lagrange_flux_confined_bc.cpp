      ...
      //fluid face 
  		...
  		const std::vector<Point>& face_normals_f = fe_face_f->get_normals();
			...
			for (unsigned int qp=0; qp<qface_f->n_points(); qp++)
			{
				...
				for (unsigned int l=0; l<phi_face_f.size(); l++)			
				{
					constraint += phi_face_f[l][qp]*last_non_linear_soln.current_local_solution->el(dof_indices_c[l]);
			 ...
			 if( rX(2)<0.0001 || rX(1)>0.9999 || rX(1)<0.0001  || rX(0)>0.9999 || rX(0)<0.0001   )
		  	{
			...
			for (unsigned int i=0; i<phi_face_f.size(); i++){	
			    Fc(i) +=  JxW_face_f[qp]*fluid_vel(0)*normal(0)*phi_face_f[i][qp];
			    Fc(i) +=  JxW_face_f[qp]*fluid_vel(1)*normal(1)*phi_face_f[i][qp];
			    Fc(i) +=  JxW_face_f[qp]*fluid_vel(2)*normal(2)*phi_face_f[i][qp];
			   // Fc(i) +=  constraint*JxW_face_f[qp]*phi_face_f[i][qp];

				Fx(i) +=  constraint*JxW_face_f[qp]*normal(0)*phi_face_f[i][qp];
				Fy(i) +=  constraint*JxW_face_f[qp]*normal(1)*phi_face_f[i][qp];
				Fz(i) +=  constraint*JxW_face_f[qp]*normal(2)*phi_face_f[i][qp];
